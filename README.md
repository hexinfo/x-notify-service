...
`body` 使用 Markdown 正文，包括粗体、标题、列表、引用、行内代码、代码块和换行；例如 `**加粗**`。旧 HTML 子集不再兼容：`<b>`、`<font>`、`<span style>` 和 `<br>` 不再作为格式语法，原样 HTML 会显示为文本。链接只显示样式，不会打开浏览器；图片和远程资源不会加载，图片仅显示替代文本。正文颜色由 `bodyTextColor` 统一控制，不支持正文内设置颜色或字号。弹窗正文超出可见区域时会裁切；弹窗常驻不超时，新通知顶掉旧的(不堆叠)。尺寸缺省仍为 220×100，四个颜色字段及其现有默认值保持不变。

自绘弹窗使用 iced tiny-skia 的 CPU 渲染，不依赖 Vulkan/GPU。Linux 自绘窗口使用 X11；没有可用 X11 显示时走系统通知。Windows 和 Linux 的桌面行为仍需在目标机器确认。
...
- Linux 自绘弹窗依赖 X11；没有可用 X11 显示时走系统通知
