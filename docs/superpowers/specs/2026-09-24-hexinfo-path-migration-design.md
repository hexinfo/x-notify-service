...
6. Linux `install.sh` 双击执行时，成功和失败都必须给出可见结果，并保持原始退出码。优先使用包内 `x-notify-service notify` 显示常驻弹窗；该程序不能显示时尝试系统已有的图形对话工具或系统通知。不要求用户额外安装软件。终端打印只用于诊断，不算完成反馈。
...
- Linux 安装脚本在成功、复制失败时展示可见的完成/失败反馈；二进制不可执行时尝试系统已有对话/通知工具。无图形会话时仍保留非零退出码和诊断文字，但不把它算作双击反馈通过。
