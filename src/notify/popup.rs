...
/// 标题单行截断:iced 0.14 的 Text 无 elide,截断在数据层完成。
pub fn elide_title(title: &str, width: f64) -> String {
    ...
    for (idx, ch) in title.char_indices() {
        // 省略号预占 1 单位,避免截断后仍超宽
        if units + char_units(ch) > max_units - 1.0 {
...
mod tests {
    ...
    use super::{
        MAX_H, MAX_W, MIN_H, MIN_W, Size, ease_out_cubic, elide_title, landing, resolve_size,
        should_retry_x11_init,
    };
