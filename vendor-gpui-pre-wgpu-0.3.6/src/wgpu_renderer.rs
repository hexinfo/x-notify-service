...
impl WgpuRenderer {
        ...
        W: HasWindowHandle + HasDisplayHandle + std::fmt::Debug + Send + Sync + Clone + 'static,
    {
            ...
            None => match WgpuContext::new(instance, &surface, compositor_gpu) {
                ...
                Err(primary_error) => {
                    #[cfg(all(target_os = "linux", target_arch = "aarch64"))]
                    {
                        ...
                        if matches!(window_handle.as_raw(), RawWindowHandle::Xcb(_)) {
                            log::warn!(
                                "XCB graphics surface failed ({primary_error}); retrying through Xlib EGL"
                            );
                            USE_XLIB_COMPAT.store(true, Ordering::Release);
                            let fallback_instance = WgpuContext::instance(Box::new(window.clone()));
                            let fallback_surface =
                                create_surface(&fallback_instance, window_handle.as_raw())?;
                            let fallback_context = WgpuContext::new(
                                fallback_instance,
                                &fallback_surface,
                                compositor_gpu,
                            )
                            .map_err(|error| {
                                anyhow::anyhow!(
                                    "XCB graphics surface failed: {primary_error}; Xlib EGL retry failed: {error}"
                                )
                            })?;
                            ctx_ref.insert(fallback_context);
                            fallback_surface
                        } else {
        ...
        let context = ctx_ref.as_ref().expect("graphics context was created");
...
fn create_surface(
    ...
    let raw_window_handle = {
        ...
        match raw_window_handle {
            RawWindowHandle::Xcb(handle) if USE_XLIB_COMPAT.load(Ordering::Acquire) => {
                RawWindowHandle::Xlib(XlibWindowHandle::new(u64::from(handle.window.get())))
            }
            handle => handle,
        }
    };
