...
impl raw_window_handle::HasDisplayHandle for XlibDisplaySource {
    fn display_handle(
        ...
        // An Xlib display with no pointer asks EGL to open DISPLAY itself.
        let raw = RawDisplayHandle::Xlib(XlibDisplayHandle::new(None, self.screen));
    ...
    pub fn instance(display: Box<dyn wgpu::wgt::WgpuHasDisplayHandle>) -> wgpu::Instance {
        ...
        let display: Box<dyn wgpu::wgt::WgpuHasDisplayHandle> = {
            ...
            if USE_XLIB_COMPAT.load(Ordering::Acquire) {
                match display.display_handle().map(|handle| handle.as_raw()) {
                    Ok(RawDisplayHandle::Xcb(xcb)) => {
                        Box::new(XlibDisplaySource { screen: xcb.screen })
                    }
                    _ => display,
                }
            } else {
